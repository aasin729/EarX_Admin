...
(function ($) {
  ...
  var DisplayPreviewDialog = function (preview, endelem) {
    var previewbackground = $(
      '<div class="ff_fileupload_dialog_background"></div>',
    );
    var previewclone = preview.clone(true, true).click(function (e) {
      e.stopPropagation();
    });
    var previewdialog = $(
      '<div class="ff_fileupload_dialog_main"></div>',
    ).append(previewclone);
  ...
  $.fn.FancyFileUpload = function (options) {
    ...
    var AddFile = function (uploads, e, data) {
      ...
      if (!data.ff_info.errors.length) {
        ...
        inforow
          .find('.ff_fileupload_actions_mobile')
          .append(
            $('<button>')
              .addClass('ff_fileupload_start_upload')
              .attr('type', 'button')
    ...
    var UploadFailed = function (e, data) {
      if (
        ...
        data.ff_info.fileinfo.text(
          FormatStr(
            Translate('{0} | Network error, retrying in a moment...'),
            data.ff_info.displayfilesize,
          ),
        );
    ...
    var UploadDone = function (e, data) {
      if (!data.result.success) {
        ...
        if (typeof data.result.errorcode !== 'string')
          data.result.errorcode = Translate('server_response');
